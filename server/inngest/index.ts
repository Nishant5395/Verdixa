...
// export const inngest = new Inngest({ id: "Instacart" });
...
//     }
//     let sentCount=0;
//     //Send in batchs of 10 to avoid overwhelming mail server 
//     const batchSize=10;
...
import { ACTIVE_DELIVERY_STATUSES, abandonUnpaidOrder, appendHistory, TERMINAL_STATUSES } from "../utils/orderHelpers.js";
...
//Auto-Assign Rider after 5 Minutes (retries every 5 minutes if every rider is busy)
...
const autoAssignRider=inngest.createFunction({
    ...
    //Try up to 6 times so an order is never left waiting forever when all riders are busy
    for(let attempt=0;attempt<6;attempt++){
        const result:AssignResult=await step.run(`assign-rider-${attempt}`,async():Promise<AssignResult>=>{
            const order=await prisma.order.findUnique({where:{id:orderId}})

            //Skip if order doesn't exist,already assigned,or finished
            if(!order)return {skipped:true,reason:"Order not found"};
            if(order.deliveryPartnerId)return {skipped:true,reason:"Already assigned"};
            if(TERMINAL_STATUSES.includes(order.status as string))return {skipped:true,reason:`Order is ${order.status}`};
            ...
            if(!availableRider)return {skipped:true,reason:NO_RIDERS}

            //Secure 6-digit OTP
            const otp=String(randomInt(100000,1000000));

            //Only assign if nobody (e.g. an admin) assigned the order in the meantime
            const claimed=await prisma.order.updateMany({
                where:{id:orderId,deliveryPartnerId:null,status:{notIn:TERMINAL_STATUSES}},
                data:{
                    deliveryPartnerId:availableRider.id,
                    deliveryOtp:otp,
                    status:"Assigned",
                    statusHistory:appendHistory(order.statusHistory,"Assigned",`Auto-assigned to ${availableRider.name}`),
                }
            })
            if(claimed.count===0)return {skipped:true,reason:"Already assigned"};
        ...
        if(!(result.skipped && result.reason===NO_RIDERS))return result;
        await step.sleep(`wait-for-rider-${attempt}`,"5m");
    }
    return {skipped:true,reason:"No riders became available"}
})

//Release stock held by card checkouts that were never paid (runs every 15 minutes)
const cleanupUnpaidOrders=inngest.createFunction({
    id:"cleanup-unpaid-orders",
    name:"Release Stock Of Abandoned Card Orders",
    triggers:[cron("*/15 * * * *")]
},async({step})=>{
    const ids=await step.run("find-abandoned-orders",async()=>{
        //Stripe sessions expire after 30 minutes, so 45 minutes is safely abandoned
        const cutoff=new Date(Date.now()-45*60*1000);
        const rows=await prisma.order.findMany({
            where:{paymentMethod:"card",isPaid:false,createdAt:{lt:cutoff}},
            select:{id:true},
            take:200,
        })
    ...
    for(const id of ids){
        await step.run(`release-${id}`,async()=>{
            await abandonUnpaidOrder(id)
        })
    }
    return {cleaned:ids.length}
})

export const functions = [checkLowStock,sendMonthlyOffers,autoAssignRider,cleanupUnpaidOrders];
