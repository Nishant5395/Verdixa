...
import type { Address, OrderQuote } from "../types";
...
const Checkout = () => {
  ...
  // ---- Prices ----
  // The server is the source of truth (it knows the coupon rules, tax and delivery fee).
  // Until its answer arrives (or if it fails) we show a local estimate.
  ...
  const cartKey = cartPayload.map((i) => `${i.product}:${i.quantity}`).join("|");

  const estimatedDelivery = cartTotal > 20 ? 0 : 1.99;
  const estimatedTax = cartTotal * 0.08;

  const subtotal = quote?.subtotal ?? cartTotal;
  const discount = quote?.discount ?? 0;
  const deliveryFee = quote?.deliveryFee ?? estimatedDelivery;
  const tax = quote?.tax ?? estimatedTax;
  const total = quote?.total ?? cartTotal + estimatedDelivery + estimatedTax;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (cancelled) return;
        if (data.couponError) {
          // e.g. the cart changed and no longer meets the coupon's minimum
  ...
  const applyCoupon = async (code: string): Promise<string | null> => {
    try {
      ...
      if (data.couponError) return data.couponError;
      setQuote(data);
      setCouponCode(data.coupon?.code ?? code);
      toast.success("Coupon applied");
      return null;
    } catch (error) {
      return errorMessage(error, "Could not apply this coupon");
    }
  };
  ...
  const handlePlaceOrder = async () => {
    if (loading) return; // stops double-clicks from placing two orders

    if (quote?.stockIssues?.length) {
      const issue = quote.stockIssues[0];
      toast.error(
        issue.available > 0
          ? `Only ${issue.available} of ${issue.name} left. Please update your cart.`
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-zinc-200 shadow-sm p-6 sticky top-24">
              ...
              {/* Stock warning (from the server's live stock check) */}
              {!!quote?.stockIssues?.length && (
                <div className="mb-5 rounded-2xl bg-red-50 border border-red-100 p-3 space-y-1">
                  {quote.stockIssues.map((issue) => (
                    <p key={issue.product} className="text-xs text-red-600">
                      {issue.available > 0
                        ? `Only ${issue.available} of ${issue.name} left in stock`
